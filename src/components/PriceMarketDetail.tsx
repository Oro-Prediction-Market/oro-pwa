...
export const PriceMarketDetail: FC<Props> = ({
  ...
  if (stakedPool > 0) {
    ...
    const lmsrOf = (o: (typeof outs)[number] | undefined) => {
      const v = Number(o?.lmsrByCurrency?.[viewerCcy] ?? o?.lmsrProbability);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Main grid: chart (left) + info sidebar (right) ── */}
        <div
          style={{
            display: "grid",
            gridTemplateColumns: isDesktop ? "minmax(0, 1.65fr) minmax(320px, 1fr)" : "1fr",
            gap: 14,
            alignItems: "start",
          }}
        >
