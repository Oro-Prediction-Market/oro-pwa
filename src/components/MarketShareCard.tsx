...
interface MarketShareCardProps {
  ...
  botUsername?: string;
  referralId?: string;
  /** Uses the same visual language as the market detail screen. */
  theme?: MarketShareTheme;
}

const BOT_USERNAME =
  (import.meta.env.VITE_BOT_USERNAME as string) || "OroPredictBot";
...
export const MarketShareCard: FC<MarketShareCardProps> = (props) => {
  ...
  const themeStyle: Record<MarketShareTheme, React.CSSProperties> = {
    ...
    epl: {
      background:
        "radial-gradient(circle at 100% 0%, rgba(0,255,133,0.20), transparent 34%), linear-gradient(135deg, #38003c 0%, #1a0020 55%, #0a0410 100%)",
      border: "1px solid rgba(0,255,133,0.34)",
      boxShadow: "inset 0 3px 0 #e90052",
    },
    ucl: {
      background:
        "radial-gradient(circle at 85% 12%, rgba(43,107,255,0.42), transparent 29%), radial-gradient(circle at 12% 100%, rgba(224,69,123,0.20), transparent 32%), linear-gradient(135deg, #0c1746 0%, #0a1235 56%, #070d29 100%)",
      border: "1px solid rgba(43,107,255,0.52)",
      boxShadow: "inset 0 2px 0 rgba(232,199,102,0.72)",
    },
    ufc: {
      // The sheet provides the red/blue fight framing. This stays a single,
      // restrained glass pane so that colour softly shows through instead of
      // creating a second competing frame.
      background:
        "linear-gradient(145deg, rgba(255,255,255,0.10) 0%, rgba(15,14,17,0.66) 34%, rgba(10,10,12,0.78) 100%)",
      border: "1px solid rgba(255,255,255,0.24)",
      backdropFilter: "blur(16px) saturate(130%)",
      WebkitBackdropFilter: "blur(16px) saturate(130%)",
      boxShadow: "inset 0 1px 0 rgba(255,255,255,0.13), 0 10px 28px rgba(0,0,0,0.22)",
    },
    esports: {
      background:
        "radial-gradient(circle at 50% -10%, rgba(242,197,117,0.19), transparent 45%), linear-gradient(180deg, #16150f 0%, #0f0e0b 100%)",
      border: "1px solid rgba(190,158,89,0.58)",
      clipPath: "polygon(12px 0, 100% 0, 100% calc(100% - 12px), calc(100% - 12px) 100%, 0 100%, 0 12px)",
    },
    btc: {
      background:
        "linear-gradient(145deg, rgba(247,147,26,0.15) 0%, rgba(20,20,31,0.82) 35%, rgba(12,12,20,0.9) 100%)",
      border: "1px solid rgba(247,147,26,0.32)",
      backdropFilter: "blur(16px) saturate(125%)",
      WebkitBackdropFilter: "blur(16px) saturate(125%)",
      boxShadow: "inset 0 1px 0 rgba(255,255,255,0.10), 0 10px 28px rgba(0,0,0,0.24)",
    },
    ter: {
      background:
        "linear-gradient(145deg, rgba(244,175,57,0.16) 0%, rgba(26,21,11,0.82) 35%, rgba(14,11,6,0.92) 100%)",
      border: "1px solid rgba(244,175,57,0.36)",
      backdropFilter: "blur(16px) saturate(125%)",
      WebkitBackdropFilter: "blur(16px) saturate(125%)",
      boxShadow: "inset 0 1px 0 rgba(255,255,255,0.10), 0 10px 28px rgba(0,0,0,0.24)",
    },
  };
  const leagueLabel: Record<MarketShareTheme, string> = {
    default: "🔮 ORO · PREDICTION MARKET",
    epl: "PREMIER LEAGUE · ORO",
    ucl: "✦ UEFA CHAMPIONS LEAGUE · ORO",
    ufc: "UFC · FIGHT MARKET",
    esports: "ESPORTS WORLD CUP · ORO",
    btc: "₿ BITCOIN · PRICE MARKET",
    ter: "TER · PRICE MARKET",
  };
  ...
  const shareButtonBackground: Record<MarketShareTheme, string> = {
    default: "linear-gradient(135deg, #2563eb, #1d4ed8)",
    epl: "linear-gradient(135deg, #00c968, #00a854)",
    ucl: "linear-gradient(135deg, #2b6bff, #173e9e)",
    ufc: "#f5f5f5",
    esports: "radial-gradient(122% 179% at 50% 0%, #f2c575 0%, #987c4b 41%, #4e442d 93%)",
    btc: "linear-gradient(135deg, #f7931a, #d86f09)",
    ter: "linear-gradient(135deg, #F4AF39, #c47d15)",
  };
            ...
            return (
              <div
                ...
                style={{
                  ...
                  alignItems: "center",
                  gap: 10,
                  padding: theme === "ufc" ? "8px 9px" : 0,
                  background:
                    theme === "ufc" ? `${barColor}18` : theme === "esports" ? "rgba(255,255,255,0.025)" : undefined,
                  border:
                    theme === "ufc" ? `1px solid ${barColor}66` : theme === "esports" ? "1px solid rgba(190,158,89,0.2)" : undefined,
                  clipPath:
                    theme === "esports"
                  ...
                  >
                    <div
                      style={{
                        ...
                        borderRadius: 3,
                        background: theme === "esports"
                          ? "linear-gradient(90deg, #987c4b, #f2c575)"
                          : barColor,
                      }}
                    />
                  </div>
