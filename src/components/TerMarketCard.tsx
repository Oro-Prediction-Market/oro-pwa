...
const TerSparkline: FC<{ history: number[]; refPrice: number }> = memo(
  ({ history, refPrice }) => {
    ...
    // When history ticks in, save only the previous last price and start a fresh tween
    useEffect(() => {
    ...
    // 60 fps render loop — runs for the component's lifetime
    useEffect(() => {
      const DURATION = 400;
      const TENSION  = 0.35;
      const GOLD     = "#F4AF39";
        ...
        if (canvas) {
          ...
          if (W > 0 && H > 0) {
            ...
            if (ctx && s.curr.length >= 2) {
              ...
              // Only animate the newest (last) point — all historical points stay locked
              ...
              // 5-point moving average — rounds staircase steps from polling
              ...
              // Right column for Y labels, bottom strip for X-axis timestamps
              const LPAD   = 52;
              const BPAD   = 16;
              const chartW = W - LPAD;
              const chartH = H - BPAD;
              ...
              // Target dashed line + label in the label column
...
interface TerMarketCardProps {
  market: Market;
  onBet?: (marketId: string, outcomeId: string, amount?: number) => void;
}

export const TerMarketCard: FC<TerMarketCardProps> = memo(
  ({ market, onBet }) => {
    ...
    // The viewer's own book — see BtcMarketCard for the same reasoning.
        ...
        <div style={{ padding: "10px 16px 14px", display: "flex", flexDirection: "column", gap: 10, marginTop: "auto" }}>
          {isSettled ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <button
                onClick={(e) => { e.stopPropagation(); if (upOutcome && onBet) onBet(market.id, upOutcome.id); }}
              ...
              <button
                onClick={(e) => { e.stopPropagation(); if (downOutcome && onBet) onBet(market.id, downOutcome.id); }}
