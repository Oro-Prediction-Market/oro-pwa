...
function parseGroupInfo(title: string): { team: string; group: string } {
        ...
        {(market.outcomes ?? [])
          ...
          return (
            <div
              ...
              style={{
                background: "linear-gradient(135deg, rgba(167,139,250,0.04) 0%, var(--bg-card, #1a1a1a) 60%)",
          ...
          return (
            <button
              ...
              style={{
                ...
                alignItems: "center",
                gap: 5,
                background: "linear-gradient(135deg, rgba(167,139,250,0.08) 0%, var(--bg-card, #1a1a1a) 70%)",
            ...
            >
              ...
              <span style={{ fontSize: 13, fontWeight: 800, color: "var(--text-main, #fff)", textAlign: "center", lineHeight: 1.15 }}>
                {outcome.label}
              </span>
...
export function WorldCupHubPage() {
  ...
  // open/upcoming ones whose betting deadline is still in the future, plus
  // closed/resolving ones (a market awaiting resolution shouldn't just vanish —
  // the card renders these locked with a "Closed"/"Resolving" badge and no
  // Predict button). We still hide resolved/settled markets: those would render
  // a misleading "Closed" countdown with an active Predict button, because the
  // card's `locked` flag only covers closed/resolving.
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-main, #0f0f0f)" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative", maxWidth: 860, margin: "0 auto" }}>
          <button
            onClick={() => navigate(-1)}
            style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 10, fontWeight: 800, color: "#A78BFA", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>
              FIFA World Cup 2026
            </div>
      ...
      {/* ── Tabs ───────────────────────────────────────────────────── */}
      <div style={{ display: "flex", borderBottom: "1px solid var(--glass-border, rgba(255,255,255,0.08))", background: "var(--bg-card, #1a1a1a)", position: "sticky", top: 0, zIndex: 10 }}>
        {TABS.map(({ key, label, icon }) => (
          <button
            ...
            style={{
              ...
              background: "none",
              border: "none",
              borderBottom: tab === key ? "2.5px solid #A78BFA" : "2.5px solid transparent",
              color: tab === key ? "#A78BFA" : "var(--text-muted, #888)",
          ...
          (Object.keys(byGroup).length === 0 && propMarkets.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
              ...
              {Object.keys(byGroup).length > 0 && (
                <div>
                  <div style={{ fontSize: 13, fontWeight: 900, color: "var(--text-main, #fff)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14, paddingBottom: 8, borderBottom: "1px solid rgba(167,139,250,0.18)" }}>
                    Groups
                  </div>
                      ...
                      .map(([group, gMarkets]) => (
                        <div key={group}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                            <span style={{ background: "#A78BFA", color: "#000", borderRadius: 6, padding: "2px 8px", fontSize: 12, fontWeight: 900 }}>
                              {group}
                            </span>
                            <span style={{ fontSize: 11, fontWeight: 800, color: "#A78BFA", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                              Group {group}
                            </span>
                          </div>
                          {gMarkets.map((market) => (
