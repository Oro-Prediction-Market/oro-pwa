  ...
  return (
    <div>
      ...
      {matchView === "upcoming" ? (
        ...
        // render of this view is always mid-request. Saying "no finished
        // matches yet" here would be wrong every time.
...
export function NationsLeagueHubPage() {
  ...
  /** Finished matches — null until someone opens Previous. See loadFinished. */
  ...
  /**
   * The finished matches, fetched the first time someone asks to see them.
   *
   * Kept apart from `markets` rather than merged into it: the live list is
   * refetched after every bet, and a single slice would have each refresh
   * overwrite the history — or force a merge that has to get de-duplication
   * right on every path.
   */
  const loadFinished = useCallback(() => {
    if (finishedReq.current) return;
    // Latched before the request, not after, so a double-tap on the toggle
    // cannot start the 10MB fetch twice.
    ...
    getMarkets()
      ...
      .catch(() => {
        // Let them try again — this only runs on an explicit tap.
