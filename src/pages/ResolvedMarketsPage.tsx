...
function EvidencePanel({ m }: { m: ResolvedMarket }) {
  ...
  function openDisputeSheet(e: React.MouseEvent) {
    ...
    if (disputes.length === 0) {
      getDisputes(m.id).then(setDisputes).catch(() => {});
    }
  }
  ...
  return (
    <div
      style={{ borderTop: "1px solid var(--glass-border)", paddingTop: 10, display: "flex", flexDirection: "column", gap: 8 }}
    ...
    >
      ...
      {hasDispute && (
        <button
          ...
          style={{
            ...
            background: m.outcomeChanged ? "rgba(239,68,68,0.08)" : "rgba(245,158,11,0.08)",
        ...
        >
          ...
          <span style={{ flex: 1, fontSize: "0.72rem", fontWeight: 700, color: m.outcomeChanged ? "#ef4444" : "#d97706", lineHeight: 1.4 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          <button
            ...
            style={{
              ...
              background: "rgba(217,119,6,0.08)", border: "1px solid rgba(217,119,6,0.3)", borderRadius: 6,
              padding: "4px 10px", cursor: "pointer", color: "#d97706",
          ...
          {expanded && (
            <div style={{ background: "var(--bg-secondary)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
              {m.evidence.note && (
                <p style={{ margin: 0, fontSize: "0.78rem", color: "var(--text-muted)", lineHeight: 1.5 }}>{m.evidence.note}</p>
              )}
              {m.evidence.url && (
                <a href={m.evidence.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
              ...
              {m.evidence.submittedAt && (
                <div style={{ fontSize: "0.68rem", color: "var(--text-subtle)" }}>
                  Submitted {new Date(m.evidence.submittedAt).toLocaleDateString("en-BT", { timeZone: "Asia/Thimphu", year: "numeric", month: "short", day: "numeric" })}
                </div>
              )}
              <button
                onClick={(e) => { e.preventDefault(); shareText(m.title, `Resolution evidence for "${m.title}"${m.evidence.url ? `\n${m.evidence.url}` : ""}`); }}
      ...
      {disputeSheet && (
        <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDisputeSheet(false); }}
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              ...
              <button onClick={(e) => { e.preventDefault(); setDisputeSheet(false); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", padding: 4 }}>
            ...
            {disputes.length === 0 && <p style={{ fontSize: 12, color: "var(--text-subtle)", textAlign: "center", padding: "20px 0" }}>Loading…</p>}
            {disputes.map((d) => (
              <div key={d.id} style={{ borderTop: "1px solid var(--glass-border)", padding: "12px 0" }}>
                ...
                {d.reason && <p style={{ margin: 0, fontSize: 13, color: "var(--text-muted)", lineHeight: 1.5 }}>{d.reason}</p>}
              </div>
            ))}
            <button
              onClick={(e) => { e.preventDefault(); shareText(m.title, `"${m.title}" had ${m.objectionCount} objection${m.objectionCount !== 1 ? "s" : ""} on Oro${m.outcomeChanged ? " — outcome was revised after review" : " — original outcome upheld"}.`); }}
