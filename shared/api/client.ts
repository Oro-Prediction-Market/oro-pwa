 ...
 * is only refreshed at login, which a long session never reaches — so a dead
 * link would otherwise paint a broken-image icon. On failure we retry once
 * through {@link avatarUrl}, which re-resolves the photo server-side and always
 * answers with an image (falling back to generated initials).
 *
